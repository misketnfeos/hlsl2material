...
class Parser:
    ...
    def parse_statement(self) -> Optional[ASTNode]:
        ...
        # 类型声明 (may return a list for multi-variable declarations)
    ...
    def parse_var_declaration(self):
        """Parse variable declaration(s): float3 color = expr;

        Supports multi-variable declarations:
            float i, s, d;
            float3 p = expr, ro = p, Z = expr;

        Returns a single VarDeclaration or a list of VarDeclaration nodes.
        """
        ...
        # Handle comma-separated additional declarators
        while self.match(TokenType.COMMA):
            self.advance()  # consume ','
            name_tok2 = self.expect(TokenType.IDENTIFIER)
            init2 = None
            if self.match(TokenType.EQUALS):
                self.advance()
                init2 = self.parse_expression()
            declarations.append(VarDeclaration(
                line=name_tok2.line, col=name_tok2.col,
                type_name=type_name,
                var_name=name_tok2.value,
                initializer=init2
            ))
